...
void agregarObjeto(Objeto inventario[], int &tam) {
    ...
    // chequeando si existe el objeto
    for (int i = 0; i < tam; ++i) {
        if (inventario[i].nombre == nombre) {
            // si existe en el inventario, se actualiza agregandole
            inventario[i].cantidad += cantidad;
            cout << "Cantidad actualizada para el objeto existente en el inventario." << endl;
            return; 
        }
    }
...
void agregarAnimal(Animal inventario[], int &tam) {
    ...
    // chequeando si existe el animalucho
    for (int i = 0; i < tam; ++i) {
        if (inventario[i].nombre == nombre) {
            // Si el animalucho existe, se actualiza la cantidad
            inventario[i].cantidad += cantidad;
    ...
    // If the animal is not found, add it to the inventory
    inventario[tam].nombre = nombre;
    inventario[tam].cantidad = cantidad;
    ++tam;
